function hh = ploterr_ik(axes_handle, x, y, xerr, yerr, varargin)
%PLOTERR General error bar plot.
%
% 2011/05/20 Modified by Ian Kleckner to take axes_handle as argument for plotting
%
%
...
if ~isempty(xl) % x errorbars
    ...
    h = plotfct(axes_handle, barx,bary,esymbol); hold on
end
if ~isempty(yl) % y errorbars
    [barx,bary]=barline(x,yl,yh,logx,hhy);
    h = [plotfct(axes_handle, barx,bary,esymbol);h]; hold on
end
if ~isempty(y) % function values
    h = [plotfct(axes_handle, x,y,symbol);h];
end
